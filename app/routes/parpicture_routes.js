...
const Parpicture = require('../models/parpicture')
...
router.post('/paris-posts-pictures', requireToken, upload.single('picture'), (req, res, next) => {
  console.log('this is my req.file when upload', req.file)
  console.log('this is my req.user when text', req.user)
  s3Upload(req.file)
    .then(awsFile => {
      return Parpicture.create({ url: awsFile.Location, owner: req.user.id, title: req.body.title, list: req.body.list, type: 'picture', upvote: 0, upvoteUserId: [1] })
    })
...
// INDEX aka GET all
router.get('/paris-posts-pictures', requireToken, (req, res, next) => {
...
router.get('/paris-posts-pictures', (req, res, next) => {
  // find all pictures where the privacy of the owner is false
  // if the owner is getting the pictures, show them their pictures as well
  Parpicture.find()
    .then(handle404)
...
//   Parpicture.find({ owner: req.user.id })
//     .then(handle404)
//     .then(pictures => {
//       pictures = pictures.map(picture => picture.toObject())
//       return Promise.all(pictures.map(picture => {
...
// // SHOW aka get by id
router.get('/paris-posts-pictures/:id', (req, res, next) => {
  Parpicture.findById(req.params.id)
    .then(handle404)
...
router.patch('/paris-posts-pictures/:id', removeBlanks, (req, res, next) => {
...
// DELETE
router.delete('/paris-posts-pictures/:id', requireToken, (req, res, next) => {
  Parpicture.findById(req.params.id)
    .then(handle404)
    .then(picture => {
      requireOwnership(req, picture)
      picture.deleteOne()
      Parpicture.deleteOne()
    })
    .then(() => res.sendStatus(204))
    .catch(next)
})
