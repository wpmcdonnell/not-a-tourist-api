...
const Lapicture = require('../models/lapicture')
...
router.post('/la-posts-pictures', requireToken, upload.single('picture'), (req, res, next) => {
  console.log('this is my req.file when upload', req.file)
  console.log('this is my req.user when text', req.user)
  s3Upload(req.file)
    .then(awsFile => {
      return Lapicture.create({ url: awsFile.Location, owner: req.user.id, title: req.body.title, list: req.body.list, type: 'picture', upvote: 0, upvoteUserId: [1] })
    })
...
// INDEX aka GET all
router.get('/la-posts-pictures', requireToken, (req, res, next) => {
...
router.get('/la-posts-pictures', (req, res, next) => {
  // find all pictures where the privacy of the owner is false
  // if the owner is getting the pictures, show them their pictures as well
  Lapicture.find()
    .then(handle404)
...
//   Lapicture.find({ owner: req.user.id })
//     .then(handle404)
//     .then(pictures => {
//       pictures = pictures.map(picture => picture.toObject())
//       return Promise.all(pictures.map(picture => {
...
// // SHOW aka get by id
router.get('/la-posts-pictures/:id', (req, res, next) => {
  Lapicture.findById(req.params.id)
    .then(handle404)
...
router.patch('/la-posts-pictures/:id', removeBlanks, (req, res, next) => {
...
// DELETE
router.delete('/la-posts-pictures/:id', requireToken, (req, res, next) => {
  Lapicture.findById(req.params.id)
    .then(handle404)
    .then(picture => {
      requireOwnership(req, picture)
