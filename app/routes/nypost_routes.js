...
const Nypost = require('../models/nypost')
...
// INDEX
// GET /posts
router.get('/ny-posts', requireToken, (req, res, next) => {
  Nypost.find()
    .then(posts => {
...
router.get('/ny-posts/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Nypost.findById(req.params.id)
    .then(handle404)
...
// CREATE
// POST /posts
router.post('/ny-posts', requireToken, (req, res, next) => {
  // set owner of new post to be current user
  req.body.post.owner = req.user.id

  Nypost.create(req.body.post)
...
router.patch('/ny-posts/:id', requireToken, removeBlanks, (req, res, next) => {
...
router.delete('/ny-posts/:id', requireToken, (req, res, next) => {
  Nypost.findById(req.params.id)
    .then(handle404)
    .then(post => {
