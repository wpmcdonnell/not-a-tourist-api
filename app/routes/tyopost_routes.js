...
const Tyopost = require('../models/tyopost')
...
// INDEX
// GET /posts
router.get('/tokyo-posts', requireToken, (req, res, next) => {
  Tyopost.find()
    .then(posts => {
...
router.get('/tokyo-posts/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Tyopost.findById(req.params.id)
    .then(handle404)
...
// CREATE
// POST /posts
router.post('/tokyo-posts', requireToken, (req, res, next) => {
  // set owner of new post to be current user
  req.body.post.owner = req.user.id

  Tyopost.create(req.body.post)
...
router.patch('/tokyo-posts/:id', requireToken, removeBlanks, (req, res, next) => {
...
router.delete('/tokyo-posts/:id', requireToken, (req, res, next) => {
  Tyopost.findById(req.params.id)
    .then(handle404)
    .then(post => {
