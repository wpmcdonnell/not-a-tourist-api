...
const Dcpicture = require('../models/dcpicture')
...
router.post('/dc-posts-pictures', requireToken, upload.single('picture'), (req, res, next) => {
  console.log('this is my req.file when upload', req.file)
  console.log('this is my req.user when text', req.user)
  s3Upload(req.file)
    .then(awsFile => {
      return Dcpicture.create({ url: awsFile.Location, owner: req.user.id, title: req.body.title, list: req.body.list, type: 'picture', upvote: 0, upvoteUserId: [1] })
    })
...
router.get('/dc-posts-pictures', (req, res, next) => {
  // find all pictures where the privacy of the owner is false
  // if the owner is getting the pictures, show them their pictures as well
  Dcpicture.find()
    .then(handle404)
...
//   Dcpicture.find({ owner: req.user.id })
//     .then(handle404)
//     .then(pictures => {
//       pictures = pictures.map(picture => picture.toObject())
//       return Promise.all(pictures.map(picture => {
...
// // SHOW aka get by id
router.get('/dc-posts-pictures/:id', (req, res, next) => {
...
// DELETE
router.delete('/dc-posts-pictures/:id', requireToken, (req, res, next) => {
  Dcpicture.findById(req.params.id)
    .then(handle404)
    .then(picture => {
      requireOwnership(req, picture)
