...
const Lapost = require('../models/lapost')
...
// INDEX
// GET /posts
router.get('/la-posts', requireToken, (req, res, next) => {
  Lapost.find()
    .then(posts => {
...
router.get('/la-posts/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Lapost.findById(req.params.id)
    .then(handle404)
...
// CREATE
// POST /posts
router.post('/la-posts', requireToken, (req, res, next) => {
  // set owner of new post to be current user
  req.body.post.owner = req.user.id

  Lapost.create(req.body.post)
...
router.patch('/la-posts/:id', requireToken, removeBlanks, (req, res, next) => {
...
router.delete('/la-posts/:id', requireToken, (req, res, next) => {
  Lapost.findById(req.params.id)
    .then(handle404)
    .then(post => {
